...
Our `output_schema` is the output data as it was _guessed_ by Socrata. Guessing may not always be correct, which is why we have import configs to "lock in" a schema for automation. We can get the `output_schema`
like so:
...
Disable SSL checking. Note that this should _only_ be used while developing
...
ArgSpec
    ...
    Defaults: metadata={}, permission=None
```
